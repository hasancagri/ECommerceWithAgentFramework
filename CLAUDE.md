...
| `file` | — | DB'siz kapak deposu (Mail.Mcp emsali); kitap kapağını ISBN ile kalıcı yerel diske (`{root}/covers/{isbn}` + `.ct` sidecar) tutar, `GET /files/v1/covers/{isbn}` anonim servis; bir-kez idempotent migration (config-gated hosted service, xlsx→indir→yaz); depolama `IFileStore` ardında (S3/MinIO sonraki spec); gateway `/files/**` anon | `specs/081-cover-image-store` |
