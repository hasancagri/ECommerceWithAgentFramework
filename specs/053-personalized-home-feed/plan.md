...
feature store'unda toplar, kullanıcının **zevk profilini** (ilgi kümeleri + öznitelik ağırlıkları + oran +
gerekçe) üretir. **Ranking .NET Storefront'ta** kalır (katalog sahibi): profili aday+skor+MMR+hidrat ile sıralı
...
`domain`/`pipeline` skorlama/kümeleme/IDF/MMR saf — test-first)

**Target Platform**: Linux/container, tek Aspire AppHost (Python dahil `AddUvicornApp`)

**Performance Goals**: Ana sayfa ilk ekran algılanır gecikme yaratmadan (SC-006); faz-1 profil istek-anında
türetilir (tek-kullanıcı GROUP BY, ucuz); ranking bellek-içi (aday havuzu SQL-filtreyle küçük). Precompute = faz-2

**Constraints**: Faz-1 = içerik-tabanlı aritmetik + basit agregasyon (gerçek ML eğitimi YOK); BC izolasyonu
(servisler-arası tek kanal = broker event + sanksiyonlu REST); async tazelik gecikmesi kabul
...
- **İlke III — VSA + CQRS:** ✅ Storefront yeni **Query** slice `GetRecommendedProducts` (POST, body=profil),
  `IQuerySession` doğrudan, repository yok. Python VSA (features/) — CQRS töreni zorlanmaz (bkz conventions).
...
- **İlke VI — Domain-TDD:** ✅ Storefront `RecommendationScoring` (skor+MMR) + Python `domain`/`pipeline` (ağırlık/
  kümeleme/IDF/oran) saf → test-first.
...
├── src/reco_trainer/
│   ├── features/
│   │   ├── ingest_signals/       # FastStream consumer + FastAPI HTTP ingest + store (1a)
│   │   └── build_profile/        # pipeline.py SAF (ağırlık/kümeleme/IDF/oran) + endpoint (1b)  ← test-first
│   ├── domain/                   # value object + saf skorlama yardımcıları
│   ├── adapters/                 # SQLAlchemy repo (signal + dedup_key), FastStream consumer (PurchaseEnriched)
│   ├── config.py  app.py
├── tests/   FLOW.md
