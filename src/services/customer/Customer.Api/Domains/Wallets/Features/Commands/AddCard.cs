            ...
            CancellationToken ct)
        {
            ...
            if (!Wallet.IsExpiryInFuture(cmd.ExpiryMonth, cmd.ExpiryYear, now))
                return FeatureObjectResultModel<AddCardResponse>.Error(
                    new MessageItem { Property = nameof(cmd.ExpiryYear), Code = CustomerResourceConstants.INVALID_VALUE });
            ...
            if (!token.Success)
                return FeatureObjectResultModel<AddCardResponse>.Error(
                    new MessageItem { Property = nameof(cmd.Pan), Code = token.ErrorCode ?? CustomerResourceConstants.INVALID_OPERATION_ERROR });
...
public static class AddCardCommandEndpoint
{
    ...
    {
        group.MapPost("", async ([FromBody] AddCardBody body, HttpContext httpContext, ICurrentUser currentUser, IMessageBus bus) =>
            {
