...
// Endpoint YOK: açılış seeder'ı, kullanıcı-akışı değil (İLKE V N/A; JIT iskelet). Süreç-güdümlü (BC'nin
// KENDİ dayanıklı süreci) — Domains/ dışına, servis kökü `Process/`'e taşındı (birden fazla aggregate'e
// dokunduğu için tek bir aggregate klasörüne ait değil; Order Saga/Process ayrımı deseni).
